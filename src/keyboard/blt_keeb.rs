// based on code from "https://github.com/taks/esp32-nimble/blob/develop/examples/ble_keyboard.rs"
extern crate alloc;
...
impl BLKeyboard {
    pub fn new() -> Self {
        ...
        hid.manufacturer("github.com/calacuda");
        ...
        Self {
            _server: server,
            input_keyboard,
            _output_keyboard: output_keyboard,
    ...
    //     self._server.connected_count() > 0
    // }
    ...
    /// if you need to press a char key, call this method with key as the char  
    pub fn press_char(&mut self, key: u8) {
        info!("pressing key: {:?}", key);
        let mut key_code = ASCII_MAP[key as usize];
        ...
        self.key_report.keys.insert(key_code);
    ...
    pub fn release(&mut self, key: u8) {
        ...
        self.key_report.keys.remove(&key_code);
        self.send_report(&self.key_report);
    }
    ...
    //     self.key_report.keys = HashSet::new();
...
impl Keyboard for BLKeyboard {
    /// all initialization is handled by the "new" method of this struct.
    fn init(&mut self) -> anyhow::Result<()> {
        info!("bluetooth keyboard initialized");
        Ok(())
    }
